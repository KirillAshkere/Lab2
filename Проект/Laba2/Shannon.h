...
{
private:
	List<bool> encoded_string; // Хранит закодированную в 0 и 1 строку
	map<char, float, List<bool>> map; //Хранит ключ, его встречаемость (значение 1), код символа (значеие 2)
	List<bool> code_symbols; // Список, который хранит код, символа
	List<pair<char, float>> symbols_and_their_count; // Список, который хранит сами символы, а также количество этих символов в строке
...
List<char> Shannon_Fano::decode(string str)
{
...
void Shannon_Fano::convert(string str)
{
	if (str.empty())
		throw runtime_error("Input string is empty");
	input_string = str;
...
List<bool> Shannon_Fano::getEncodedString()
{
	return encoded_string;
}

void Shannon_Fano::display()
{
	if (input_string.empty())
...
void Shannon_Fano::set_codes(size_t indexL, size_t indexR)
{
	if (indexR - indexL == 2)
	{
